...
const [products, setproducts] = useContext(ProductContext);
 ...
 <div className={`w-full h-full flex justify-center ${!product ? "bg-red-100" : " "} `}>
    {product ?
    <div className='w-[80%] h-full border shadow hover:shadow-2xl flex justify-center items-center m-auto p-10 gap-8 '>{console.log(product)}
      <img src={`${product.image}`} className='object-contain w-[30%] h-[57%]' alt="loading" />
      <div className='w-[40%] flex flex-col text-wrap gap-1  '>
        <h1 className='text-3xl font-semibold text-wrap '>{product.title}</h1>
        <h3 className='text-red-300 '>{product.category}</h3>
        <h2 className='text-xl font-semibold'>{product.price} (inr)</h2>
        <p className='text-balance mb-[5%] text-lg'>{product.description}</p>
